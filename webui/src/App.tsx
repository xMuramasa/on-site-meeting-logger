...
function ReviewForm({ review, update, busy, save, finalize }: { review: api.Review; update: (fn: (value: api.Review) => api.Review) => void; busy: boolean; save: () => void; finalize: () => void }) {
  return <div className="review-stack">
    <section className="review-card"><div className="section-title"><div><span className="step">02</span><h2>Confirmaciones humanas</h2></div><Radio size={21} /></div>
      ...
      {!!Object.keys(review.relative_dates).length && <><h3>Fechas relativas</h3><div className="field-grid">{Object.entries(review.relative_dates).map(([id, dueDate]) => <label key={id}><span>{id}</span><input type="date" value={dueDate || ""} onChange={(e) => update((value) => ({ ...value, relative_dates: { ...value.relative_dates, [id]: e.target.value } }))} /></label>)}</div></>}
